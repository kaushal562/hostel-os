...
interface PremiumRangeCalendarPickerProps {
  dateRange?: DateRange;
...
export function PremiumRangeCalendarPicker({
  dateRange,
  onDateRangeChange,
  placeholder = "Pick a date range",
  className,
  disabled = false,
}: PremiumRangeCalendarPickerProps) {
  return (
    <Popover>
          ...
          {dateRange?.from ? (
            dateRange.to ? (
              <span className="text-[#f0f4ff]">
                ...
                <span className="text-[#00d9ff]">{format(dateRange.to, "MMM dd, yyyy")}</span>
              </span>
            ) : (
              <span className="text-[#00d9ff]">{format(dateRange.from, "MMM dd, yyyy")}</span>
            )
          ) : (
