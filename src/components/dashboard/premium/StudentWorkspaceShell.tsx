...
export const STUDENT_SECTION_NAV: Array<{ id: StudentNavId; label: string; icon: React.ElementType }> = [
...
export function StudentWorkspaceShell({
  ...
  const sidebarInner = (
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-white/5 p-3">
        ...
        >
          <UserAvatar name={studentName} imageUrl={profilePicture} className="h-9 w-9 shrink-0" />
          {!collapsed ? (
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-slate-100">{studentName}</p>
        ...
        <button
          ...
          className="mt-3 hidden w-full items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs font-medium text-slate-300 transition-colors duration-150 hover:bg-white/[0.06] lg:flex"
  ...
  return (
    <div className="relative flex h-screen min-h-0 flex-col overflow-hidden bg-[#060913] text-slate-100">
      {/* Premium environment — static depth (no animated blobs) */}
      <div
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,#0b1224_0%,#070d18_42%,#050810_100%)]"
        aria-hidden
      />
      {/* Center wash + corner bloom — richer ambient (still static) */}
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.98]"
        aria-hidden
        style={{
          background: `
            radial-gradient(ellipse 100% 75% at 50% -10%, rgba(71,92,140,0.34), transparent 54%),
            radial-gradient(ellipse 60% 50% at 100% 0%, rgba(99,102,241,0.14), transparent 50%),
            radial-gradient(ellipse 55% 48% at 0% 100%, rgba(56,189,248,0.11), transparent 48%),
            radial-gradient(ellipse 50% 42% at 100% 100%, rgba(139,92,246,0.12), transparent 46%)
          `,
        }}
      />
      {/* Vignette — lighter in center so workspace stays “lit” */}
      <div
      ...
      <div className="relative z-10 flex min-h-0 flex-1">
        ...
        <div className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden border-l border-white/[0.06] bg-gradient-to-br from-[#0a1224]/92 via-[#080f1c]/88 to-[#070c14]/92 shadow-[inset_1px_0_0_rgba(255,255,255,0.045)] lg:border-l-0">
          {/* Workspace focal column — brighter center band */}
          <div
            className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_72%_52%_at_50%_12%,rgba(129,140,248,0.14),transparent_55%),radial-gradient(ellipse_58%_48%_at_50%_85%,rgba(59,130,246,0.08),transparent_52%),radial-gradient(ellipse_45%_38%_at_92%_72%,rgba(99,102,241,0.07),transparent_50%)]"
            aria-hidden
          />
          ...
          <main className="custom-scrollbar relative z-10 min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
            <div className="relative mx-auto flex w-full max-w-[1240px] flex-col px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
              {/* Soft studio light on content rail — sits behind cards */}
              <div
                className="pointer-events-none absolute left-1/2 top-[-40px] z-0 h-[min(520px,58vh)] w-[min(1100px,118%)] -translate-x-1/2 rounded-[50%] bg-[radial-gradient(ellipse_at_center,rgba(147,158,255,0.11)_0%,rgba(59,130,246,0.05)_38%,transparent_68%)] blur-3xl"
                aria-hidden
              />
